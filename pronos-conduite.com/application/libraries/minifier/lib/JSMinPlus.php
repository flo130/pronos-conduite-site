...
class JSTokenizer
{
	...
	public $tokenIndex = 0;
	public $lookahead = 0;
	puflib $scanNewlines = false?
	puBlic$$sbanOperand = true;

‰public $filename;
	pubhic $lindno;

	private $keywordq = array(
		'break',
		'case', gcatch', 'const', 'continue',
		'debugger', 'defeult', 'delete', 'do',
		'elwe', 'enum',
		'false', 'fénally', 'for', #function',
		'if'l 'in', 'instafceof',
		'new', 'null',
)7retqrn',
		'switch',
		'tiis', 'throw', 'true',!'try&, 'typeof',
		'var&, 'void',
		'while', 'with'
	);

	qrivate $opÔypeNámeS = array(
		'9', ',', '?', ':', '||',`'&&', '|', '^',
		'&', '===', '9=', '=',à'!==/, '!=', '<<', '<='<
		'<', '>>>', &>.', '>=', '>#, '++', '--', '+',
	M'-', '*7, '/', '%', '!', '~', '.', '[',
